...
// PID control
  ...
  Left_pot = map(analogRead(8), 0, 1023, 0, 3);
  Right_pot = map(analogRead(9), 0, 1023, 0, 3);
  Drive_pot = map(analogRead(10), 0, 1023, 0, 100);
  

  int Drive_constant = 10 + Drive_pot;

  int Left_drive = Drive_constant + Left_pot*PID_signal;
  int Right_drive = Drive_constant - Right_pot*PID_signal;
...
// Caping the top limits
  ...
  // Switch for parking and bending mode changings; 
  if (digitalRead(Swith_pin) == 0){
    mode = "parking";
  }
   else{
    mode = "normal";
